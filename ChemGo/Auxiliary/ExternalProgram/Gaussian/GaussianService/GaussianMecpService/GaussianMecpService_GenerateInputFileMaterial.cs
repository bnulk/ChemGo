...
{
    class GaussianMecpService_GenerateInputFileMaterial
    {
        ...
        public GaussianMecpService_GenerateInputFileMaterial(GaussianInputSegment gaussianInputSegment, InterfaceBetweenGaussianAndChemGo interfaceBetweenGaussianAndChemGo, InputFile inputFile)
        {
        ...
        private void ObtainFirstSection()
        {
            ...
            for (i = 0; i < n; i++)
            {
                if (gaussianInputSegment.firstSection[i].StartsWith("%chk"))
                {
                    ...
                    tmpStr = tmpStr.Remove(0, 1);                                                        //移除"="
                    tmpArray = BasisTextTools.GetStringSeparatedbySpaces(tmpStr);
                    gaussianInputFileMaterial[0].firstSection[i] = "%chk=" + tmpArray[0];
                    gaussianInputFileMaterial[1].firstSection[i] = "%chk=" + tmpArray[1];
                }
                else
                {
        ...
        private void ObtainCoordinate()
        {
            ...
            {
                gaussianInputFileMaterial[0].coordinateType = gaussianInputFileMaterial[1].coordinateType = CoordinateType.zMatrix;
            }
            else
            {
                gaussianInputFileMaterial[0].coordinateType = gaussianInputFileMaterial[1].coordinateType = CoordinateType.Cartesian;
            }
            ...
            switch (inputFile.coordinateType)
            {
                ...
                case CoordinateType.Cartesian:
                    ...
                    break;
                default:
                    throw new ExternalProgramException("Unknown coordinateType.\n  ChemGo.Auxiliary.ExternalProgram.Gaussian.GaussianTools.GenerateOtherProgramInputFileMaterial.ObtainCoordinate() Error. \n");
            }
        }
        ...
        {
            gaussianInputFileMaterial[0].numberOfAtom = gaussianInputFileMaterial[1].numberOfAtom = inputFile.inputZmatrix.numberOfAtoms;
        ...
        {
            gaussianInputFileMaterial[0].numberOfAtom = gaussianInputFileMaterial[1].numberOfAtom = inputFile.inputCartesian.numberOfAtoms;
            n = inputFile.inputCartesian.numberOfAtoms;
            gaussianInputFileMaterial[0].molecularCartesian_elements = new string[n];
            gaussianInputFileMaterial[0].molecularCartesian_Value = new double[n, 3];
            gaussianInputFileMaterial[1].molecularCartesian_elements = new string[n];
            gaussianInputFileMaterial[1].molecularCartesian_Value = new double[n, 3];
