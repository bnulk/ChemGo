...
namespace ChemGo.Auxiliary.LinearAlgebra
{
    class LinearEquation
    {
       ...
       (1) gaussj(ref Matrix a, ref Vector b)  
           解线性方程组。 Ab=c
           a是输入矩阵，b为包含m个右端向量的输入矩阵。输出时，a被其逆矩阵代替，b为相应的解向量代替。
       ----------------------------------------------------  类注释  结束----------------------------------------------------
       */

        /// <summary>
        /// 描述：* 完全主元法的Gauss-Jordan消去法。  
        /// 方法：* gaussj(ref Matrix a, ref Vector b)  a是输入矩阵，b为包含m个右端向量的输入矩阵。输出时，a被其逆矩阵代替，b为相应的解向量代替。
        /// 代码来源：* c++数值计算(第二版)
        /// </summary>
        /// <param name="a"></param>
        /// <param name="b"></param>
        public static void gaussj(ref Matrix a, ref Vector b)
        {
            ...
            for (i = 0; i < n; i++)                                  //约化列的主循环
            {
                ...
                if (irow != icol)
                {
                    ...
                    Util.SWAP(ref b.ele[irow], ref b.ele[icol]);
                }
